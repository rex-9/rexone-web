...
export const NeonSign: React.FC<INeonSignProps> = ({
  ...
  return (
    <section
      id={id}
      className={`flex justify-center items-center w-full max-w-2xl min-h-40 sm:min-h-56 mx-auto my-12 tracking-wider sm:tracking-widest select-none uppercase font-display text-5xl sm:text-7xl md:text-8xl lg:text-9xl leading-none text-center text-glow-white animate-hero-sign [text-shadow:0_0_7px_var(--color-glow-white),0_0_10px_var(--color-glow-white),0_0_21px_var(--color-glow-white),0_0_42px_var(--color-primary),0_0_82px_var(--color-primary),0_0_92px_var(--color-primary),0_0_102px_var(--color-primary),0_0_151px_var(--color-primary)] ${className}`}
      style={{
        backgroundImage: `radial-gradient(ellipse 65% 55% at 50% 50%, rgba(var(--color-glow-outer-rgb), 0.85), transparent 70%)`,
      }}
    >
      ...
      <span>o</span>
      <span>n</span>
      <span className="animate-fast-flicker-char">e</span>
    </section>
  );
};
