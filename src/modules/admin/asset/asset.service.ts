...
import { IApiEnvelope, IApiResponse, IAsset, IAssetUploadResponse } from "../../../models";
...
class AdminAssetService {
  ...
  async uploadAsset(
    ...
    if (options?.assetable_type) formData.append("assetable_type", options.assetable_type);
    if (options?.assetable_id) formData.append("assetable_id", options.assetable_id);
  ...
  async updateAsset(
    id: string,
    data: Partial<Pick<IAsset, "name" | "type" | "assetable_type" | "assetable_id">>,
  ): Promise<IApiResponse<IApiEnvelope<{ asset: IAdminAsset }>>> {
  ...
  async destroyAsset(
    id: string,
  ): Promise<IApiResponse<IApiEnvelope<void>>> {
